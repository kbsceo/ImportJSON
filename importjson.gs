function UPDATEJSON_1() {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    // Type your sheet name to push data
    var sheetStore = ss.getSheetByName("raw_data");

    var scriptNo = [675272, 675273, 675274];
    var result = [];

    for (var i = 0; i <= scriptNo.length; i++) {
        var numRows = sheetStore.getLastRow();
        var numColumns = sheetStore.getLastColumn();
        console.log(numRows);
        console.log(numColumns);
        console.log(convertDateFormat(new Date()));

        // Example URL
        var url = "https://www.kbsceo.com/servlet/API/JSON/sNo/" + scriptNo[i] + "/STATS?mediaId=example&targetDate=" + convertDateFormat(new Date()) + "&productType=all&statsType=media";

        var result = ImportJSON(url, "", "noHeaders");
        if (numRows == 0) {
            sheetStore.getRange(i + 1, 1, result.length, result[0].length).setValues(result);
        } else {
            sheetStore.getRange(numRows + i + 1, 1, result.length, result[0].length).setValues(result);
        }
...
function UPDATEJSON() {
    ...
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetStore = ss.getSheetByName("realclick_raw_data");
    var result = [];
    var numRows = sheetStore.getLastRow();
    var numColumns = sheetStore.getLastColumn();
    ...
    // Example URL
    var url = "https://media.kbsceo.xyz/syndi_static.php?media=0K0BS9CEO&rdate=" + convertDateFormat(new Date());
    console.log(url);

    var result = ImportJSON(url, "", "noHeaders");

    if (numRows == 0) {
        sheetStore.getRange(1, 1, result.length, result[0].length).setValues(result);
    } else {
        sheetStore.getRange(numRows + 1, 1, result.length, result[0].length).setValues(result);
    }
}

// Delete row if specific data in each cell 
function deleteSpecificRows() {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var s = ss.getSheetByName('mobon_raw_data');
...
// Delete EmptyRows if specific cell is empty
...
// Check the cell if cell is empty
function cellEmptyRow() {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var s = ss.getSheetByName('mobon_raw_data');
    var lastRow = s.getLastRow();

    for (var i = lastRow; i > 1; i--) {
        var range = s.getRange("G:G");
        var data = range.getValue();
        if (data == '') {
            s.setValue('0');
        }
...
// Convert date format like YYYYMMDD
...
// Under things are ImportJSON Library that is external.
...
function ImportJSONAdvanced(url, query, options, includeFunc, transformFunc) {
    ...
    var object = JSON.parse(jsondata.getContentText("euc-kr"));
